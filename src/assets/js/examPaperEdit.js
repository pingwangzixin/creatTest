...
export default {
	...
	data() {
		return {
			//word格式文件
//			dataObj : dataObj,
			pageOperation : '',		//页面状态 editVolume：编辑卷面  knowledgePoint
			...
			knowledgePointsDifficulty : [	//知识点难度 （固定）
				{name:"困难",id:"difficult5"},
				{name:"较难",id:"difficult4"},
				{name:"一般",id:"difficult3"},
				{name:"较易",id:"difficult2"},
				{name:"容易",id:"difficult1"}
			],
			knowledData : [],
			//所有数据提交用
			allData : {
			...
			smallDataFormat : {
				...
				"questionContentCopy" : [],		//
				"knowledgePoint" : {			//知识点/难易度
					"degree" : 3,
					"point" : [
//						{"kpointId" : "","kpointName" : ""}
					]
				},
				"quzSection" : [],				//小题选择题选择区间/判断题区间
				"quzType" : "",					//小题类型
				"split" : "",					//小题拆分状态
				"state" : "",					//小题编辑状态
				"titNo" : "",					//小题序号
...
//						console.log(contMap)
		...
		//知识点数据请求加载
		knowledDataLoad (){
			//{"levelId":"level_2","subjectId":"subject_2_2"}
			wordRelevant.getKnowledData({"levelId":"level_2","subjectId":"subject_2_2"}).then(res => {
				console.log(res);
				if(res.status == 200){
					console.log(this)
					console.log(res.data.data)
					console.log(res.data.data[0].children)
					this.knowledData = res.data.data[0].children;
					console.log(this.knowledData);
				}
			}).catch(e => {
		...
		knowledgeStateAll (e){
			//批量
			this.pageStatic.knowledgePoints.batch = true;
			this.pageStatic.knowledgePoints.left = 680;
			this.pageStatic.knowledgePoints.top = this.getPos(e.target).top - 30;
			//清除联动框状态
			this.clearKnowledgeState();
			if(this.allData.pageRelated.knowledge && this.pageStatic.knowledgePoints.display){  //有结构有框
				this.allData.pageRelated.knowledge = false;
				this.pageStatic.knowledgePoints.display = false;
			}else if(this.allData.pageRelated.knowledge && this.pageStatic.knowledgePoints.display == false){ //有结构没框
				this.pageStatic.knowledgePoints.display = true;
			}else{ //没结构没框
				this.allData.pageRelated.knowledge = true;
				this.pageStatic.knowledgePoints.display = true;
			}
		...
		knowledgeState (){
			//非批量
			this.pageStatic.knowledgePoints.batch = false;
			this.allData.pageRelated.knowledge = !this.allData.pageRelated.knowledge;
		},
		//添加知识点——联动框关闭
		knowledgeHide(){
//			['click','scroll'].forEach((item,i) => {
			　　 document.addEventListener('click',(e) => {
					if(this.$refs.knowledge_linkage){
						//是否包含当前点击元素
						if((!this.$refs.knowledge_linkage.contains(e.target) && e.srcElement.innerText != '+添加') && (!this.$refs.knowledge_linkage.contains(e.target) && e.srcElement.innerText != '批量添加知识点') ){
						 	this.pageStatic.knowledgePoints.display  = false;
						}
					}
			　　  });
...
//				console.log(e)
		...
		//添加知识点按钮
		addKnowledge (e,indexFF,indexF,index){
//			console.log(this.pageStatic.knowledgePoints.btnObj)
//			console.log(e.currentTarget == this.pageStatic.knowledgePoints.btnObj)
			//非批量
			this.pageStatic.knowledgePoints.batch = false;
			if(e.currentTarget == this.pageStatic.knowledgePoints.btnObj){
				this.pageStatic.knowledgePoints.display = !this.pageStatic.knowledgePoints.display;
			}else{
				this.pageStatic.knowledgePoints.display = true;
			}
//			this.pageStatic.knowledgePoints.top = e.clientY + 20;
			this.pageStatic.knowledgePoints.left = 68;
			this.pageStatic.knowledgePoints.top = this.getPos(e.target).top - 30;
			//清除联动框状态
			this.clearKnowledgeState();
			
			this.pageStatic.knowledgePoints.btnObj = e.currentTarget;
			//拿到插入对象
			this.pageStatic.knowledgePoints.targetObj = this.allData.analysisData[indexFF].smallTitle[indexF].content[index].knowledgePoint.point;
		},
		getElementToPageTop (e) {
			if(e.parentElement) {
		   		return this.getElementToPageTop(e.parentElement) + e.offsetTop
		  	}
		 	return e.offsetTop
		},
		//获取元素距离
　　　　   getPos(obj){
        ...
        //清除联动框状态
        clearKnowledgeState (){
        	this.knowledData.forEach((ele,i) => {
				ele.state = false;  
				ele.checked = false;  
				if(ele.children.length){	//有下一级清除状态
					ele.children.forEach((sEle,sI) => {
						sEle.state = false;
						sEle.checked = false;
						if(sEle.children.length){   //有下一级继续清除状态
							sEle.children.forEach((tEle,tI) => {
								//清除checkbox状态,最后一级无需清除active状态
								tEle.checked = false;	
							});
						}
					});
		...
		knowledgeF (index){
			this.knowledData.forEach((ele,i) => {
				ele.state = false;
				if(i == index){//选中
					ele.state = true;   //选中active状态以及下一级展示
					if(ele.children.length){	//有下一级清除状态
						ele.children.forEach((sEle,sI) => {
							sEle.state = false;
							sEle.checked = false;
...
//						console.log(this.pageStatic.knowledgePoints.targetObj)
						if(this.pageStatic.knowledgePoints.batch){  //批量
							this.allData.analysisData.forEach((e,i) => {
								e.smallTitle.forEach((e2,i2) => {
									e2.content.forEach((e3,i3) => {
										let result = e3.knowledgePoint.point.some((item,index) => {
											return item.kpointName == ele.name && item.kpointId == ele.id; 
										});
										console.log(result)
										if(!result) e3.knowledgePoint.point.push({kpointId:ele.id,kpointName:ele.name});
									});
								});
							});
						}else{
							let result = this.pageStatic.knowledgePoints.targetObj.some((e,i) => {
								return e.kpointName == ele.name && e.kpointId == ele.id; 
							});
							if(!result) this.pageStatic.knowledgePoints.targetObj.push({kpointId:ele.id,kpointName:ele.name});
						}
					}
				}
			});
		...
		knowledgeS (e,index){
			e.stopPropagation();
			this.knowledData.forEach((ele,i) => {
				if(ele.state){   //找到一级选中节点
					ele.children.forEach((sEle,sI) => {  
						sEle.state = false;
						if(sI == index) {//选中
							sEle.state = true;		//选中active状态以及下一级展示
							if(sEle.children.length){		//有下一级继续清除状态
								sEle.children.forEach((tEle,tI) => {
									//清除checkbox状态,最后一级无需清除active状态
									tEle.checked = false;	
								...
								if(this.pageStatic.knowledgePoints.batch){  //批量
									this.allData.analysisData.forEach((e,i) => {
										e.smallTitle.forEach((e2,i2) => {
											e2.content.forEach((e3,i3) => {
												let result = e3.knowledgePoint.point.some((item,index) => {
													return item.kpointName == sEle.name && item.kpointId == sEle.id; 
												});
												console.log(result)
												if(!result) e3.knowledgePoint.point.push({kpointId:sEle.id,kpointName:sEle.name});
											});
										});
									});
								}else{
									let result = this.pageStatic.knowledgePoints.targetObj.some((e,i) => {
										return e.kpointName == sEle.name && e.kpointId == sEle.id;
									});
									if(!result) this.pageStatic.knowledgePoints.targetObj.push({kpointId:sEle.id,kpointName:sEle.name});
								}
							}
						}
					});
									...
									if(this.pageStatic.knowledgePoints.batch){  //批量
										this.allData.analysisData.forEach((e,i) => {
											e.smallTitle.forEach((e2,i2) => {
												e2.content.forEach((e3,i3) => {
													let result = e3.knowledgePoint.point.some((item,index) => {
														return item.kpointName == tEle.name && item.kpointId == tEle.id; 
													});
													console.log(result)
													if(!result) e3.knowledgePoint.point.push({kpointId:tEle.id,kpointName:tEle.name});
												});
											});
										});
									}else{
										let result = this.pageStatic.knowledgePoints.targetObj.some((e,i) => {
											return e.kpointName == tEle.name && e.kpointId == tEle.id;
										});
										if(!result) this.pageStatic.knowledgePoints.targetObj.push({kpointId:tEle.id,kpointName:tEle.name});
									}
								}
							});
						}
		...
		deleteKnowledge (item,index){
			//this.allData.analysisData[0].smallTitle[1].content[0].knowledgePoint
			item.splice(index,1);
			console.log(item)
		},
		//小题号排序
...
// 			//console.log(this.allData.analysisData);
 		...
 		smallSingleChoose (allObj,currentObj){
 			if(this.pageOperation=='editVolume'){
...
//              	headInfo : this.$route.query.headInfo 	//考试信息相关
 		...
 		submitData (){
			...
			if(this.pageOperation=='editVolume'){	//编辑 -->去答案页
				if(this.findWordImg ()){
		...
		//							paperResId : res.data.data.paperResId
				...
				this.allData.analysisData.forEach(function (e,i){
					e.smallTitle.forEach((groupE,groupI) => {
						groupE.content.forEach((contentE,contentI) => {
//							return contentE.knowledgePoint.point.length;
							if(contentE.knowledgePoint.point.length < 1){
								//状态（success/fail/warning）  提示
								console.log(this)
								_this.$parent.$parent.timingFn('warning','请完善所有知识点');
							}else{
								console.log(contentI)
								if(i == _this.allData.analysisData.length-1 && groupI == e.smallTitle.length-1 && contentI == groupE.content.length - 1){
									alert()
								}
							}
						});
...
//	                        paperMsg : JSON.stringify(this.allData)
	    ...
	    toQuestionlist(){
 		...
 		//习题栏按钮
        expandExerciseBar(){
            this.$parent.$parent.exerciseBar();
        },
        //习题栏添加大题
        addQuesTopage (){
//			JSON.parse(JSON.stringify(this.allData));
			
			let bigObj = JSON.parse(JSON.stringify(this.bigDataFormat));
			bigObj.headlineTitNo = this.questionOrder[this.allData.analysisData.length];
			let insertArr = this.$store.state.arr.filter((e,i) => {
				return (e.checked == true || e.checked == 1);
			});
			console.log(insertArr)
			insertArr.forEach((e,i) => {
				let groupObj = JSON.parse(JSON.stringify(this.groupDataFormat));
				let smallObj = JSON.parse(JSON.stringify(this.smallDataFormat));
				groupObj.content.push(smallObj);
				groupObj.content[0].titNo = i
				groupObj.content[0].questionContent[0] = e.body;
				groupObj.content[0].questionContentCopy[0] = e.body;
				groupObj.content[0].questionContent[1] = e.parse;
				groupObj.content[0].questionContentCopy[1] = e.parse;
				groupObj.content[0].questionContent[2] = e.answer;
				groupObj.content[0].questionContentCopy[2] = e.answer;
				bigObj.smallTitle.push(groupObj);
			});
			this.allData.analysisData.push(bigObj);
			//排序
			this.titlePatternSort(this.allData.pageRelated.titleNaturalOrder);
			console.log(this.allData.analysisData)
...
//		console.log(this.$route.query.pageState);
		console.log(this.$route.query.pageOperation);
...
//		this.wordData();
		...
		this.$store.dispatch("requestViewTitle",{levelId:sessionInfo.headInfo.levelId,subjectId:sessionInfo.headInfo.subjectId});
		
		//编辑页面 or 添加知识点
		this.pageOperation = this.$route.query.pageOperation;
        //知识点数据加载
        this.knowledDataLoad();
...
//			console.log("<div id='dd'>" + sessionStorage.getItem('newPaperMsg') + "</div>")
			...
			this.allData.analysisData.forEach((e,i) => {
				e.smallTitle.forEach((e2,i2) => {
					e2.content.forEach((e3,i3) => {
						e3.knowledgePoint = {degree : '3',point : []};
					});
				});
			});
			console.log(this.allData.analysisData)
			...
			wordRelevant.echoAllData({"paperResId" : this.allData.paperResId,"step":"2"}).then(res => {
				...
				if(res.status == 200){
//					this.allData = res.data.data;
					this.allData.analysisData = res.data.data.analysisData;
					this.allData.headInfo = res.data.data.headInfo;
					this.allData.paperResId = res.data.data.paperResId;
					this.allData.userId = res.data.data.userId;
					console.log(this.allData)
		            //大题类型
					if(this.allData.analysisData[0].sectionType == ""){
						this.allData.analysisData.forEach(function (e,i){
					...
					/*this.allData.analysisData.forEach((e,i) => {
						e.smallTitle.forEach((e2,i2) => {
							e2.content.forEach((e3,i3) => {
								e3.knowledgePoint = {degree : "1",point : [{"kpointId":"12312312",kpointName:"数与方程式"},{"kpointId":"12312312",kpointName:"数与方程式"}]};
							})
						})
					})
					this.allData.analysisData[0].smallTitle[0].content[0].knowledgePoint = {degree : "3",point : [{"kpointId":"12312312",kpointName:"数与方程式"},{"kpointId":"12312312",kpointName:"数与方程式222"}]};
					this.allData.analysisData[0].smallTitle[1].content[0].knowledgePoint = {degree : "4",point : [{"kpointId":"12312312",kpointName:"数与方程式"},{"kpointId":"12312312",kpointName:"数与方程式565"},{"kpointId":"12312312",kpointName:"这次不是方程式"}]};*/
