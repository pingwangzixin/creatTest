	...
	<div class="">
		...
		<div class="g_border">
                ...
                <ul class="task_list j_task_list">
                    <li class="big_question_list" v-for="(bigI,index) in allData.analysisData" :key="index">
                        <div class="head j_head clearfix" @mouseover="headlineIn(bigI)" @mouseout="headlineOut(bigI)">
                            ...
                            <div class="tit_cont fl" v-html="bigI.topicNumber" v-show="!bigI.headlineState"></div>
                            ...
                            <span class="btn" v-show="bigI.headlineOperation">
                                ...
                                <b class="btn_split btn_split_big" @click="headlineSplit(bigI,'big')">拆分大题</b>
                        ...
                        <ul class="cont">
                            <li class="clearfix group j_group" :class="{active : groupI.groupBorder}" v-for="(groupI,groupIndex) in bigI.smallTitle" @mouseover="groupIn(groupI)" @mouseout="groupOut(groupI)" :key="groupIndex">
                                <div class="group_head j_group_head" v-if="groupI.groupTitState" @mouseover="groupTitIn(groupI)" @mouseout="groupTitOut(groupI)">
                                    ...
                                    <div class="tit_cont" v-html="groupI.group" v-show="!groupI.groupState"></div>
                                     <vue-ueditor-wrap class="editor_unified" :ref="(index - 0 + 1) + '-' + (groupIndex - 0 + 1)" :config="myConfig" v-model="groupI.groupCopy" v-show="groupI.groupState"></vue-ueditor-wrap>
                                </div>
                                <div class="question j_question" v-for="(quesI,quesIndex) in groupI.content" @mouseover="smallIn(quesI)" @mouseout="smallOut(quesI)" :key="quesIndex">
                                    <span class="btn fr" v-show="quesI.btnOperation">
                                        ...
                                        <!--<b class="btn_merge_group j_btn_merge_group" v-if="groupI.group == '' && (groupIndex > 0 || bigI.smallTitle[groupIndex - 1].content = [])">与上题合并为题目组</b>-->
                                        <b class="btn_merge_group j_btn_merge_group" v-if="groupI.group == '' && groupIndex > 0" @click="mergeGroup(quesI,quesIndex,index,groupIndex)">与上题合并为题目组</b>
                                        <!--<b class="btn_merge_group j_btn_merge_group" v-if="groupI.group == '' && (quesIndex > 0 || ())">与上题合并为题目组</b>-->
                                        ...
                                        <div class="question_type clearfix">
                                            ...
                                            <div class="question_template fl">
                                                ...
                                                <div class="all_tem multiple clearfix" v-if="quesI.quzType == 4">
                                                	<span class="fl">答案</span>
                                                    <ul class="fl">
                                                        <li :class="{active : i.state}" v-for="(i,optionIndex) in quesI.quzSection" v-text="i.name" :key="optionIndex" @click="smallcheckChoose(quesI,i)">A</li>
                                                        <li class="option_btn fl">
                                                            <i class="iconfont iconjia" @click="smallAddOption(quesI.quzSection)"></i>
                                            				<i class="iconfont iconjianshao" @click="smallReduceOption(quesI.quzSection)"></i>
                                                        </li>
                                                ...
                                                <div class="all_tem judge clearfix" v-if="quesI.quzType == 1">
                                                    ...
                                                    <ul class="fl">           
                                                        <li :class="quesI.questionContent[2] == '正确' ? 'active' : ''" @click="smallJudge(quesI,'正确')">对</li>
                                                        <li :class="quesI.questionContent[2] == '错误' ? 'active' : ''" @click="smallJudge(quesI,'错误')">错</li>
                                    ...
                                    <div class="qusetion_main">
                                        <div class="normal_state" v-show="!quesI.state">
                                            ...
                                            <transition name="analysis">
	                                            <div class="analysis_answer analysis_cont clearfix" v-show="allData.pageRelated.analysis">
			...
			<div class="zy_paper_title_btn">
            	<div class="btn_group">
	                <button class="cancle">取消并返回列表</button>
	                <button class="middle">上一步</button>
	                <button class="sure" @click="submitData()">设置答案</button>
            ...
            <!--各种框框-->
            <!--快捷合并题目组-->
			<div class="prompt_box public_box quick_merge_group" v-if="allData.pageRelated.fastMergeGroup.display">	
				<div class="hide"></div>
				<div class="con">
					<div class="tit">
						<span>快捷合并题目组</span>
					...
					<div class="main_cont">
						<p class="order">
							<span>起始题号：</span>
							<input type="number" name="" id="" value="" placeholder="请输入" v-model="allData.pageRelated.fastMergeGroup.start" />
						</p>
						<p class="number">
							<span>结束题号：</span>
							<input type="number" name="" id="" value="" placeholder="请输入" v-model="allData.pageRelated.fastMergeGroup.end" />
						</p>
...
export default {
	...
	data() {
		return {
			...
			allData : {
				userId : 'system',
				paperResId : '259368646f414f15826a4325306da70b',
				//头部筛选相关
				headInfo : {
					name : '今天语文',
					termName : '萨拉肯德基',
			...
			groupDataFormat : {	
				...
				"groupId" : "", 				//题组id
				"groupSeq" : "1",				//题组数据库序号
				"groupType" : "5",				//题组类型（设置分值）
				"groupScore" : "15",			//题组总分
				"groupCount" : "5", 			//题组下小题数量
				"groupSeqLable" : "A)",			//题组序号
				"groupSeqTitle" : ""			//题组标题
			},
			//小题格式
			smallDataFormat : {
...
//						contMap["articleNum"] = 0;
		...
		time (){
			this.$parent.$parent.timingFn('success','2');
		},
		confirm (){
			let _this = this;
...
//						this.titlePatternSort(this.allData.pageRelated.titleNaturalOrder);
		...
		groupFastMerge(groupI,groupIndex,index){
			...
			this.allData.pageRelated.fastMergeGroup.display = true;
			this.allData.pageRelated.fastMergeGroup.bigI = index;
			this.allData.pageRelated.fastMergeGroup.groupI = groupIndex;
		},
		//题组快速合并题目组弹框确认按钮
		groupFastMergeSure (){
			let _this = this;
			let insert = [];
			let start = this.allData.pageRelated.fastMergeGroup.start;
			let end = this.allData.pageRelated.fastMergeGroup.end;
			console.log(insert)
			this.allData.analysisData[this.allData.pageRelated.fastMergeGroup.bigI].smallTitle.forEach(function (e,i){
				...
				if(i >= start && i <= end - 1){
					...
					insert.push(JSON.parse(JSON.stringify(e.content[0])));
				}
			});
			this.allData.analysisData[this.allData.pageRelated.fastMergeGroup.bigI].smallTitle.splice(start,end - 1);
			console.log(insert)
			insert.forEach(function (e,i){
				_this.allData.analysisData[_this.allData.pageRelated.fastMergeGroup.bigI].smallTitle[_this.allData.pageRelated.fastMergeGroup.groupI].content.push(e);
			});
			console.log(this.allData.analysisData[this.allData.pageRelated.fastMergeGroup.bigI])
			this.allData.pageRelated.fastMergeGroup.display = false;
			start = this.allData.pageRelated.fastMergeGroup.start = 0;
			end = this.allData.pageRelated.fastMergeGroup.end = 0;
		},
		//题组快速合并题目组弹框取消按钮
		groupFastMergeCancel (){
			this.allData.pageRelated.fastMergeGroup.display = false;
			this.allData.pageRelated.fastMergeGroup.bigI = 0;
			this.allData.pageRelated.fastMergeGroup.groupI = 0;
		},
		//题组标题删除按钮
...
//			this.allData.analysisData[index].smallTitle[groupIndex].content.splice(quesIndex,1);
 		...
 		smallSure (quesI,questionBox,index,groupIndex,quesIndex){
			...
			if(quesI.split){//拆分状态
				let ue = this.$refs[(index - 0 + 1) + "-" + (groupIndex - 0 + 1) + "-" + (quesIndex - 0 + 1)][0].editor;
				let splitlabel = "<split-text></split-text>";
				ue.execCommand("inserthtml", splitlabel, true);
				let text = ue.body.innerHTML.substring(0,ue.body.innerHTML.indexOf(splitlabel));
				let regSplitlabel = /<split-text><\/split-text>/g;
...
//				console.log(questionBox)
				...
				obj.questionContentCopy[1] = "";
				obj.questionContentCopy[2] = "";
				questionBox.splice(quesIndex + 1,0,obj);
				//排序
				this.titlePatternSort(this.allData.pageRelated.titleNaturalOrder);
				console.log(questionBox)
			}else{//编辑状态
 		...
 		smallDelete (quesI,index,groupIndex,quesIndex){
			...
			if(this.allData.analysisData[index].smallTitle[groupIndex].content.length == 2 && this.allData.analysisData[index].smallTitle[groupIndex].groupTitState){
				alert('题组小题小于2道，将解散题组');
				this.allData.analysisData[index].smallTitle[groupIndex].group = '';
				this.allData.analysisData[index].smallTitle[groupIndex].groupCopy = '';
				this.allData.analysisData[index].smallTitle[groupIndex].groupTitState = false;
				this.allData.analysisData[index].smallTitle[groupIndex].content.splice(quesIndex,1);
			}else{
 		...
 		//小题与上题合并为题目组
 		mergeGroup (quesI,quesIndex,index,groupIndex){
...
// 			//console.log(this.allData.analysisData);
 		...
 		submitData (){
   			...
   			/*this.$router.push({
                path : '/index/process/setAnswer',
                query : {
                    paperMsg : JSON.stringify(this.allData)
                }
           	});*/
           	sessionStorage.setItem("paperMsg",JSON.stringify(this.allData));
//         	console.log(this.allData.paperResId)
           	this.$router.push({
                path : '/index/process/setAnswer',
                query : {
					paperResId : this.allData.paperResId
                }
           	});
...
//	                        paperMsg : JSON.stringify(res.data.data)
 		...
 		//获取数据
 		obtainData (){
 			//this.$route.query.paperResId
 			wordRelevant.echoAllData({"paperResId" : this.$route.query.paperResId,"step":"2"}).then(res => {
				console.log(res);
				if(res.status == 200){
